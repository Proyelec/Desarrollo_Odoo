...
- **One2many + write() timing bug**: Cuando `write()` del padre contiene cambios en un campo one2many, los registros hijos AÚN NO están persistidos cuando el cuerpo del `write()` del padre se ejecuta. `self.order_line` refleja el estado PRE-write, no el estado que el usuario acaba de guardar. Solución: usar `@api.constrains` en lugar de validar dentro de `write()` — los constrains se disparan DESPUÉS de que toda la transacción (incluyendo líneas hijas) ya está persistida. Patrón erróneo: leer `self.child_ids` dentro de `write()` para validar valores que el usuario acaba de cambiar. Patrón correcto: `@api.constrains('child_ids', 'state')` en el modelo padre.
- **group_operator=False en Float**: Campos Float que representan porcentajes o ratios calculados por-registro (como `% en este pedido`) deben declararse con `group_operator=False` para evitar que Odoo los sume al agrupar registros en list/pivot. Sin esto, al agrupar por departamento, el campo muestra la suma de todos los porcentajes de cada SO, dando valores imposibles como 535%.
- **XPath column order en tree heredada**: Si dos módulos heredan el mismo tree y el orden de los campos importa, usar `position="before"` sobre el campo de referencia en lugar de `position="replace"` para intercalar columnas. El `replace` posiciona en el lugar del campo reemplazado, que puede no ser el orden deseado.
